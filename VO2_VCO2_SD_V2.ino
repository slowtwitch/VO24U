...
const String Version = "V2.2 PUGH 2023/01/23";
...
#include <Wire.h>
#include "FS.h" // File system library - used for SD card
...
String dataMessage; // Create a String variable to hold the data to be saved on the microSD card.

// Initialize an instance of SPIClass attached to VSPI
  SPIClass * vspi = NULL; 

  // Declare fileCount globally so that it can be used throughout the code
int fileCount = 0;
...
void setup() {
    ...
    */
    ...
    if (!Serial) {
      ...
      Serial.println("Beginning SD prep");
  ...
  vspi->begin(VSPI_SCLK, VSPI_MISO, VSPI_MOSI, VSPI_SS); //SCLK, MISO, MOSI, SS
    ...
    if (!SD.begin(VSPI_SS,*vspi,1000000)) {
    Serial.println("Card Mount Failed");
    tft.drawString("SD bad", 120, 0, 4);
    //SDCardDisconnectedflag = true;
      return;}
    else {Serial.println("SD ready");
      //  SDCardDisconnectedflag = false;
      tft.drawString("SD ok", 120, 0, 4);
    }
  
delay(100); // Add a short delay before printing the message
Serial.println("Delaying post SD begin");
...
if (cardType == CARD_NONE) {
 ...
 Serial.println("SD CARD BIT STARTING");
  uint64_t cardSize = SD.cardSize() / (1024 * 1024);
  Serial.printf("SD Card Size: %lluMB\n", cardSize);

    // Read the existing count from the file
  File countFile = SD.open("/file_count.txt", FILE_READ);
  if (countFile) {
    fileCount = countFile.parseInt();
    countFile.close();
  }
  ...
  // Create or open the file to store the count of files
  countFile = SD.open("/file_count.txt", FILE_WRITE);
  if (countFile) {
    countFile.print(fileCount); // Write the updated count
    countFile.close();
    Serial.printf("File count: %d\n", fileCount);

    // Call the function to create a new file with the current count
    createNewFileWithCount();
  } else {
    Serial.println("Failed to open file_count.txt");
  }

  Serial.print("File count is ");
  Serial.println(fileCount);
  Serial.printf("Total space: %lluMB\n", SD.totalBytes() / (1024 * 1024));
  Serial.printf("Used space: %lluMB\n", SD.usedBytes() / (1024 * 1024));

  Serial.println("Finished SD prep");
    ...
    delay(2000);
 Serial.println("Pre activate Sensirion App");
 ...
 Serial.println("Post activate Sensirion App");
    CheckInitialO2();
 Serial.println("Post CheckInitialO2");
    CheckInitialCO2();
 Serial.println("Post CheckInitialCO2");


  Serial.println("Beginning menus");

    doMenu();
 Serial.println("Init BLE bit STARTING");
    showParameters();
...
void loop() {
    ...
    // VO2 calculation, tft display and excel csv every 5s --------------
...
void createNewFileWithCount() {
  ...
  // Create a new file with the current count as the name
  String newFileName = "/file_" + String(fileCount) + ".txt";
  File newFile = SD.open(newFileName, FILE_WRITE);
  if (newFile) {
    newFile.println("This is a new file!");
    newFile.close();
    Serial.printf("Created new file: %s\n", newFileName.c_str());
  } else {
...
/*
  ...
  */
    Serial.println("Forming data message string");

    // Build data message string
    dataMessage = String(float(TotalTime / 1000)) + "," +
                  String(vo2Max) + "," + String(vo2MaxMax) +
                  "," + String(vco2Max) + "," +
                  String(respq) + "," + String(volumeExp) +
                  "," + String(volumeVEmean) + "," +
                  String(freqVEmean) + "," +
                  String(lastO2) + "," +
                  String(co2perc, 3) + "," + + "\r\n";
    ...
    appendFile(SD, ("/file_" + String(fileCount) + ".txt").c_str(), dataMessage.c_str());
